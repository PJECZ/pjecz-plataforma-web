...
class SentenciaNewForm(FlaskForm):
    ...
    sentencia = StringField("Sentencia", validators=[DataRequired(), Length(max=10), Regexp(SENTENCIA_REGEXP)])
...
class SentenciaSearchForm(FlaskForm):
    ...
    sentencia = StringField("Sentencia", validators=[Optional(), Length(max=10), Regexp(SENTENCIA_REGEXP)])
