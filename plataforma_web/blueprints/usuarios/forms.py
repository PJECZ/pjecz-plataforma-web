...
CONTRASENA_REGEXP = r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$'
...
class AccesoForm(FlaskForm):
    ...
    identidad = StringField('Correo electrónico o usuario', validators=[DataRequired(), Length(4, 128)])
    contrasena = PasswordField('Contraseña', validators=[DataRequired(), Length(4, 64)])
...
class UsuarioFormNew(FlaskForm):
    ...
    contrasena_repetir = PasswordField('Repetir contraseña')
    guardar = SubmitField('Guardar')
...
class CambiarContrasenaForm(FlaskForm):
    ...
    contrasena = PasswordField('Nueva contraseña', validators=[
        DataRequired(),
        Regexp(CONTRASENA_REGEXP, 0, CONTRASENA_MENSAJE),
        EqualTo('contrasena_repetir', message='Las contraseñas deben coincidir.'),
    ])
