...
def list_active():
    ...
    viejos_mensajes = Mensaje.query.filter(Mensaje.estatus == "A").filter(Mensaje.leido == True).filter(or_(Mensaje.destinatario_id == current_user.id, Mensaje.autor == current_user.email)).limit(500).all()
