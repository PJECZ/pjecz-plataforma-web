...
def solicitar(fin_vale_id: int, contrasena: str):
    ...
    if response.status_code != 200:
        mensaje = f"Error al solicitar el vale: {response.text}"
        fin_vale.solicito_efirma_mensaje = mensaje
        fin_vale.estado = "CANCELADO POR SUPERIOR"
        fin_vale.save()
        bitacora.error(mensaje)
        return set_task_error(mensaje)
    ...
    try:
        ...
        mensaje = "Error al firmar el vale porque la respuesta no es JSON"
        fin_vale.solicito_efirma_mensaje = mensaje
        fin_vale.estado = "CANCELADO POR SUPERIOR"
        fin_vale.save()
        bitacora.error(mensaje)
