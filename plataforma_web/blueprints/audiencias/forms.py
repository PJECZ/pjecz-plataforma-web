...
class AudienciaGenericaForm(FlaskForm):
    ...
    tiempo = DateTimeField("Fecha y hora", validators=[DataRequired()])
    tipo_audiencia = StringField("Tipo de audiencia", validators=[DataRequired(), Length(64)])
    expediente = StringField("Expediente (número/año)", validators=[DataRequired(), Length(max=16), Regexp(EXPEDIENTE_REGEXP)])
    actores = StringField("Actores", validators=[DataRequired(), Length(256)])
    demandados = StringField("Demandados", validators=[DataRequired(), Length(256)])
...
class AudienciaMapoForm(FlaskForm):
    ...
    tiempo = DateTimeField("Fecha y hora", validators=[DataRequired()])
    tipo_audiencia = StringField("Tipo de audiencia", validators=[DataRequired(), Length(64)])
    sala = StringField("Sala", validators=[DataRequired(), Length(16)])
    caracter = SelectField("Caracter", choices=Audiencia.CARACTERES, validators=[DataRequired()])
    causa_penal = StringField("Causa penal", validators=[DataRequired(), Length(256)])
    delitos = StringField("Delitos", validators=[DataRequired(), Length(256)])
    guardar = SubmitField("Guardar")
...
class AudienciaDipesapeForm(FlaskForm):
    ...
    tiempo = DateTimeField("Fecha y hora", validators=[DataRequired()])
    tipo_audiencia = StringField("Tipo de audiencia", validators=[DataRequired(), Length(64)])
    toca = StringField("Toca", validators=[DataRequired(), Length(64)])
    ...
    delitos = StringField("Delitos", validators=[DataRequired(), Length(256)])
    imputados = StringField("Imputados", validators=[DataRequired(), Length(256)])
    guardar = SubmitField("Guardar")
