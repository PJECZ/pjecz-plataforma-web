...
from lib.safe_string import DIR_IP_REGEXP
...
class SIGASalaNewForm(FlaskForm):
    ...
    direccion_ip = StringField("Dirección IP", validators=[Optional(), Length(max=16), Regexp(DIR_IP_REGEXP)])
    direccion_nvr = StringField("Dirección NVR", validators=[Optional(), Length(max=16), Regexp(DIR_IP_REGEXP)])
