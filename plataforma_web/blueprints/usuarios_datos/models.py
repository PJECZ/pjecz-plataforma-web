...
class UsuarioDato(db.Model, UniversalMixin):
    ...
    estado_acta_nacimiento = db.Column(db.Enum(*VALIDACIONES, name="estado_identificacion", native_enum=False))
    estado_domicilio = db.Column(db.Enum(*VALIDACIONES, name="estado_domicilio", native_enum=False))
    estado_curp = db.Column(db.Enum(*VALIDACIONES, name="estado_domicilio", native_enum=False))
    estado_cp_fiscal = db.Column(db.Enum(*VALIDACIONES, name="estado_cp_fiscal", native_enum=False))
    estado_curriculum = db.Column(db.Enum(*VALIDACIONES, name="estado_cp_fiscal", native_enum=False))
    estado_estudios = db.Column(db.Enum(*VALIDACIONES, name="estado_estudios", native_enum=False))
    estado_acta_nacimiento_hijo = db.Column(db.Enum(*VALIDACIONES, name="estado_acta_nacimiento", native_enum=False))
    estado_estado_civil = db.Column(db.Enum(*VALIDACIONES, name="estado_estado_civil", native_enum=False))
    estado_estado_cuenta = db.Column(db.Enum(*VALIDACIONES, name="estado_estado_civil", native_enum=False))
    ...
    mensaje_acta_nacimiento_hijo = db.Column(db.String(255))
